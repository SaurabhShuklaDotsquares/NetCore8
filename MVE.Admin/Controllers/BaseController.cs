...
namespace MVE.Admin.Controllers
{
    ...
    public class BaseController : Controller
    {
        ...
        public async Task CreateAuthenticationTicket(AdminUser user, int[] userPermissionIds, string roleType, bool remember, List<Dto.RoleActionPermissionDTO> allActionPermissions, GeneralSiteSettingDTO GlobalSetting)
        {
            if (user != null)
            {
                ...
                identity.AddClaim(new Claim("allActionPermissions", JsonConvert.SerializeObject(allActionPermissions)));
                 identity.AddClaim(new Claim("SiteSettings", siteSetting));

                var props = new AuthenticationProperties();
                props.IsPersistent = remember;
                props.ExpiresUtc = DateTimeOffset.UtcNow.AddMonths(12);
        ...
        public Int32 GetSessionData()
        {
            return Convert.ToInt32(HttpContext.Session.GetInt32("ActivityIncrementCounter"));
        }
